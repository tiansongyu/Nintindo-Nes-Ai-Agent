...
def main():
    parser = argparse.ArgumentParser(description="Legacy playback entry point.")
    parser.add_argument("game_number", help="Legacy game number.")
    parser.add_argument("--IsRender", default=True, help="Legacy render flag.")
    args, extras = parser.parse_known_args()

    argv = ["play", get_game(args.game_number).slug]
    has_render_override = "--render" in extras or "--no-render" in extras
    if not has_render_override:
        if parse_bool(args.IsRender):
            argv.append("--render")
        else:
